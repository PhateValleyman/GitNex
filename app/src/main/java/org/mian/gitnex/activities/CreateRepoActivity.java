...
public class CreateRepoActivity extends BaseActivity {
	...
	private void createNewRepository(final String instanceUrl, final String token, String loginUid, String repoName, String repoDesc, String repoOwner, boolean isPrivate) {
		...
		if(repoOwner.equals(loginUid)) {
			...
			call = RetrofitClient.getInstance(instanceUrl, ctx).getApiInterface().createNewUserRepository(token, createRepository);

		}
		else {

			call = RetrofitClient.getInstance(instanceUrl, ctx).getApiInterface().createNewUserOrgRepository(token, repoOwner, createRepository);
	...
	private void getOrganizations(String instanceUrl, String instanceToken, final String userLogin) {
		...
		Call<List<OrgOwner>> call = RetrofitClient.getInstance(instanceUrl, ctx).getApiInterface().getOrgOwners(instanceToken);
			...
			public void onResponse(@NonNull Call<List<OrgOwner>> call, @NonNull retrofit2.Response<List<OrgOwner>> response) {
				...
				if(response.isSuccessful()) {
					if(response.code() == 200) {
						...
						ArrayAdapter<OrgOwner> adapter = new ArrayAdapter<>(CreateRepoActivity.this, R.layout.spinner_item, organizationsList);
